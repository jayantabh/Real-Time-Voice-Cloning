...
class Network(nn.Module):
    def __init__(self):
        ...
        # Network defition
        self.lstm = nn.LSTM(input_size=40,hidden_size=model_hidden_size,num_layers=model_num_layers,batch_first=True)#bidirectional=True)
        self.fc1 = nn.Linear(in_features=model_hidden_size, out_features=model_embedding_size)
        self.fc2 = nn.Linear(in_features=model_embedding_size, out_features=248)
        self.relu = torch.nn.ReLU()
        
    def forward(self, x, hidden_init=None):
#         print("Forward : ", x.shape, x.type())
        _, (hidden, _) = self.lstm(x, hidden_init)
#         print(hidden.shape)
